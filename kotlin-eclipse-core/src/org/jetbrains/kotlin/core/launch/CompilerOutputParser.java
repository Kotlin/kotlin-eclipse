...
public class CompilerOutputParser {
    ...
    public static void parseCompilerMessagesFromReader(MessageCollector messageCollector, final Reader reader) {
        ...
        try {
            ...
            messageCollector.report(CompilerMessageSeverity.ERROR, message, CompilerMessageLocation.getNO_LOCATION());
        }
        finally {
            try {
                reader.close();
    ...
    private static class CompilerOutputSAXHandler extends DefaultHandler {
        ...
        public void characters(char[] ch, int start, int length) throws SAXException {
            if (tags.size() == 1) {
                ...
                if (!message.trim().isEmpty()) {
                    messageCollector.report(CompilerMessageSeverity.ERROR, "Unhandled compiler output: " + message, CompilerMessageLocation.getNO_LOCATION());
                }
            }
            else {
        ...
        public void endElement(String uri, String localName, String qName) throws SAXException {
            ...
            if (category == null) {
                messageCollector.report(CompilerMessageSeverity.ERROR, "Unknown compiler message tag: " + qName, CompilerMessageLocation.getNO_LOCATION());
