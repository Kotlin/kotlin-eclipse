...
object KotlinLightClassGeneration {
    ...
    private fun checkByInternalName(internalName: String?, requestedClassFileName: String): Boolean {
        return if (internalName != null) {
            ...
            if (requestedClassFileName.startsWith(classFileName)) {
                if (requestedClassFileName.length > classFileName.length) {
                    val charAfterMainName = requestedClassFileName[classFileName.length]
                    if (charAfterMainName == '$' || charAfterMainName == '.') {
                        return true
                    }
                }
            }
