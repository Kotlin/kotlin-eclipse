...
// Copy pasted with small changes from com.intellij.formatting.SpacingImpl
public class SpacingImpl extends Spacing {
    ...
    public SpacingImpl(final int minSpaces, final int maxSpaces, final int minLineFeeds, final boolean isReadOnly,
            final boolean safe, final boolean shouldKeepLineBreaks, final int keepBlankLines,
            final boolean keepFirstColumn, final int prefLineFeeds) {
        init(minSpaces, maxSpaces, minLineFeeds, isReadOnly, safe, shouldKeepLineBreaks, keepBlankLines,
                keepFirstColumn, prefLineFeeds);
    }
    
    void init(final int minSpaces, final int maxSpaces, final int minLineFeeds, final boolean isReadOnly,
            final boolean safe, final boolean shouldKeepLineBreaks, final int keepBlankLines,
            final boolean keepFirstColumn, final int prefLineFeeds) {
        ...
        myFlags = (isReadOnly ? READ_ONLY_MASK : 0) | (safe ? SAFE_MASK : 0)
    ...
    /**
     * <b>Note:</b> current implementation uses soft type check, i.e. it checks
     * that instance of the given object IS-A {@link SpacingImpl} and compares
     * state defined at this class only. That means that sub-classes are assumed
     * not to override this method in order to preserve <code>'symmetric'</code>
     * property.
     *
     * @param o
     *            {@inheritDoc}
    ...
    public boolean equals(Object o) {
        ...
        return myFlags == spacing.myFlags && myMinSpaces == spacing.myMinSpaces && myMaxSpaces == spacing.myMaxSpaces
                && myMinLineFeeds == spacing.myMinLineFeeds && myPrefLineFeeds == spacing.myPrefLineFeeds
