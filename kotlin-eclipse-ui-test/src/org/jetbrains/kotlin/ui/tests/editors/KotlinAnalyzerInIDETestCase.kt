...
abstract class KotlinAnalyzerInIDETestCase : KotlinEditorAutoTestCase() {
    ...
    private fun loadFilesToProjectAndDoTest(files: List<File>) {
        ...
        filesWithExpectedData
            .filter { it.first.getFileExtension() == "java" }
            .map { JavaCore.create(it.first) as ICompilationUnit }
            .forEach { KotlinTestUtils.reconcileCompilationUnit(it) }
        KotlinTestUtils.joinBuildThread()
