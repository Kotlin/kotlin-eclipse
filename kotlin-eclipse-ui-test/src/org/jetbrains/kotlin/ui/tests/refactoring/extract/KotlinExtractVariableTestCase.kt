...
public abstract class KotlinExtractVariableTestCase : KotlinProjectTestCase() {
    ...
    fun doTest(testPath: String, newName: String = "i") {
        ...
        val ref = KotlinExtractVariableRefactoring(editor.getSelectionProvider().getSelection() as ITextSelection, editor)
        val monitor = NullProgressMonitor()
        
        ref.checkInitialConditions(monitor)
        ref.newName = newName
        ref.createChange(monitor).perform(monitor)
