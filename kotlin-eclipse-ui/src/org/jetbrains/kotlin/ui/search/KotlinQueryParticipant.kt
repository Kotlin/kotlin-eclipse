...
public class KotlinQueryParticipant : IQueryParticipant {
    ...
    private fun resolveElementsAndMatch(elements: List<KtElement>, searchElement: SearchElement, 
        ...
        return sortedByFileNameElements.filter { element ->
            ...
            val additionalElements = getContainingClassOrObjectForConstructor(sourceElements).singletonOrEmptyList()
...
fun getContainingClassOrObjectForConstructor(sourceElements: List<SourceElement>): SourceElement? {
    return sourceElements.singleOrNull()?.let {
        if (it is KotlinSourceElement) {
            val psi = it.psi
            if (psi is KtConstructor<*>) {
                return@let KotlinSourceElement(psi.getContainingClassOrObject())
            }
        }
