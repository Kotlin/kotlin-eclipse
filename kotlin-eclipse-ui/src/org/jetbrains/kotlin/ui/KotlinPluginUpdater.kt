...
//    private val DELAY: Long = 3 * 1000 * 60 * 60 // 3 hours
    ...
    private fun tryToUpdate(store: IPreferenceStore): PluginUpdateStatus {
        ...
        if (kotlinUnit == null) {
            return PluginUpdateStatus.UpdateFailed("There is not kotlin feature group with id: $KOTLIN_GROUP_ID")
        }
        ...
        try {
            updateSite = obtainUpdateSite("https://plugins.jetbrains.com/eclipse-plugins/kotlin/update?build=$eclipseVersion&pluginVersion=$pluginVersion&os=$os&uuid=$uid")
        } catch (e: IOException) {
