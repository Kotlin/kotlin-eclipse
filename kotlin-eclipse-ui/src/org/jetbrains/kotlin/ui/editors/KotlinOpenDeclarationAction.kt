...
class KotlinOpenDeclarationAction(val editor: KotlinEditor) : SelectionDispatchAction(editor.javaEditor.site) {
    ...
    override fun run(selection: ITextSelection) {
        ...
        val target = getTargetElement(references)
        if (target == null) return
        
        val javaProject = editor.javaProject
        if (javaProject == null) return
        
        gotoElement(target.second, target.first, javaProject)
    }
    
    private fun getTargetElement(reference: List<KotlinReference>): Pair<KotlinReference, SourceElement>? {
        return reference
                .resolveToSourceElements()
                .withIndex()
                .find { it.value != SourceElement.NO_SOURCE }
                ?.let { Pair(reference[it.index], it.value) }
    }
