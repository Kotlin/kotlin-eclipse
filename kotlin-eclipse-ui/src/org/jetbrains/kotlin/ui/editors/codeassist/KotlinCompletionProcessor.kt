    ...
    private fun generateCompletionProposals(viewer: ITextViewer, offset: Int): List<ICompletionProposal> {
        ...
        val proposals = arrayListOf<ICompletionProposal>().apply {
            if (simpleNameExpression != null) {
                ...
                val ktElement = EditorUtil.getJetElement(editor, offset)

                if (ktElement != null) {
                    val ktFile = ktElement.getContainingKtFile()

                    val bindingContext = KotlinAnalysisFileCache.getAnalysisResult(
                            ktFile).analysisResult.bindingContext
                    val declarationDescriptor = bindingContext.get(BindingContext.FUNCTION,
                            psiElement)

                    if (declarationDescriptor is CallableDescriptor) {
                        for (item in declarationDescriptor.valueParameters) {
                            val parameterName = item.name.asString();
                            val image = JavaUI.getSharedImages().getImage(org.eclipse.jdt.ui.ISharedImages.IMG_OBJS_LOCAL_VARIABLE)
                            add(KotlinCompletionProposal(parameterName, image, parameterName, parameterName, identifierPart = identifierPart))
                        }
                    }
