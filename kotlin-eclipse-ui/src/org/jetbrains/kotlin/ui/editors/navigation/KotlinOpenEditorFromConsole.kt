...
class KotlinOpenEditorFromConsole : IPatternMatchListenerDelegate {
    ...
    override fun matchFound(event: PatternMatchEvent) {
        ...
        if (validateLink(fqName, lineNumber)) {
    ...
    private fun validateLink(fqName: String, lineNumber: String): Boolean {
        val type = findType(fqName)
        if (type == null) return false
        
        val resource = type.resource
        if (resource !is IFile) return false
    ...
    private fun openEditor(fqName: String, lineNumber: String) {
        ...
        try {
            ...
            KotlinLogger.logError("Valid link should have right location position", e)
        }
