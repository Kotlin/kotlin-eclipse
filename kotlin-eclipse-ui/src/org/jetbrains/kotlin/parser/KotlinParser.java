...
public class KotlinParser {
    ...
    private final ArrayList<PsiElement> psiElements; 
    private boolean psiElementsChanged;
    ...
    public KotlinParser(File file) {
        this.file = file;
        this.psiElements = new ArrayList<>();
        this.tree = null;
        this.psiElementsChanged = false;
    }
    ...
    public ASTNode parse() {
        ...
        psiElementsChanged = true;
        
        return tree;
    }
    
    public ArrayList<PsiElement> getPsiElements() {
        if (psiElementsChanged) {
            psiElements.clear();
            setPsiElements(tree);
        }
        psiElementsChanged = false;
    ...
    private void setPsiElements(ASTNode node) {
        if (node == null) {
            return;
        }
        for (ASTNode child = node.getFirstChildNode(); child != node.getLastChildNode(); child = child.getTreeNext()) {
            if (child instanceof PsiElement) {
                psiElements.add((PsiElement) child);
            } 
            else if (child != null) {
                setPsiElements(child);
            }
        }
    }
}
